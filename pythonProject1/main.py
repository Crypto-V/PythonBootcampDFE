...
headers = {
    "User-agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (XHTML, like Gecko) Chrome/77.0.3865.120 "
                  "Safari/537.36"}
...
with open(file_path, mode='w') as file:
    ...
    for page in range(no_of_pages):
        url = 'https://www.indeed.co.uk/jobs?q=' + skill + \
              '&l=' + place + 'vjk=3cd7ccf130b5121a' + str(page * 10)
        response = requests.get(url, headers=headers)
        html = response.text

        # Scrapping the Web
        soup = BeautifulSoup(html, 'html.parser')
        base_url = 'https://www.indeed.co.uk/jobs?q='
