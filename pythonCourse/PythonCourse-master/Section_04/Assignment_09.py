...
for line in fr:
    ...
    if (d.get(lookup_val) is None):  # cant find in d

        next_number = int(max(d.values())) + 1  # taking the max number in d values
        d[lookup_val] = str(next_number)
        columns[0] = str(next_number)

        # each line will be added as a string with the value and will be appended to line_list
        line_list.append("|".join(columns))

    else:  # we were able to find the email in the dictionary
